...
utah %>%
  group_by(Pop.Group) %>%
  summarize(Mean_Religious = mean(Religious))
...
utah %>%
  group_by(Pop.Group) %>%
  summarize(Mean_Pop = mean(Pop_2010))
...
utah %>%
  group_by(Pop.Group) %>%
  summarize(Mean_Religious = mean(Religious), Mean_Pop = mean(Pop_2010)) -> religiosity
...
religiosity %>%
  arrange(desc(Mean_Pop)) -> religiosity
